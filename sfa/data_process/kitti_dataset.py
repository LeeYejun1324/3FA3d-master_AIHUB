...
class KittiDataset(Dataset):
    def __init__(self, configs, mode='trainval', lidar_aug=None, hflip_prob=None, num_samples=None):
        ...
        assert mode in ['train', 'val', 'test', 'trainval'], 'Invalid mode: {}'.format(mode)
        self.mode = mode
    ...
    def get_label(self, idx):
        ...
        for line in open(label_path, 'r'):
            ...
            h, w, l = float(line_parts[5]), float(line_parts[6]), float(line_parts[4])
            # location (x,y,z) in camera coord.
            x, y, z = -(float(line_parts[2])+0.5), float(line_parts[3])+3.14, float(line_parts[1])-1.0
            if float(line_parts[7]) <= -1.57:
                ry = float(line_parts[7])+1.57+3.141592  # yaw angle (around Y-axis in camera coordinates) [-pi..pi]
            else:
                ry = float(line_parts[7])-1.57
...
if __name__ == '__main__':
    ...
    dataset = KittiDataset(configs, mode='trainval', lidar_aug=lidar_aug, hflip_prob=0., num_samples=configs.num_samples)
    ...
    for idx in range(len(dataset)):
        ...
        for box_idx, (cls_id, x, y, z, h, w, l, yaw) in enumerate(labels):
            ...
            drawRotatedBox(bev_map, x1, y1, w1, l1, 0, cnf.colors[int(cls_id)])
        # Rotate the bev_map

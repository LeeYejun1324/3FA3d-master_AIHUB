...
class Compute_Loss(nn.Module):
    ...
    def forward(self, outputs, tg):
        ...
        l_hm_cen = self.focal_loss(outputs['hm_cen'], tg['hm_cen'].cuda())
        l_cen_offset = self.l1_loss(outputs['cen_offset'], tg['obj_mask'].cuda(), tg['indices_center'].cuda(), tg['cen_offset'].cuda())
        l_direction = self.l1_loss(outputs['direction'], tg['obj_mask'].cuda(), tg['indices_center'].cuda(), tg['direction'].cuda())
        # Apply the L1_loss balanced for z coor and dimension regression
        l_z_coor = self.l1_loss_balanced(outputs['z_coor'], tg['obj_mask'].cuda(), tg['indices_center'].cuda(), tg['z_coor'].cuda())
        l_dim = self.l1_loss_balanced(outputs['dim'], tg['obj_mask'].cuda(), tg['indices_center'].cuda(), tg['dim'].cuda())
